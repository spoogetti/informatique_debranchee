...
class ReconnectMap extends Phaser.GameObjects.Container {
    ...
    createTerrain() {
        ...
        const pipeLengthCols = this.width / numberCols
        const pipeLengthRows = this.height / numberRows
        const pipeLength = pipeLengthCols < pipeLengthRows ? pipeLengthCols : pipeLengthRows
    ...
    createCols(numberRows, numberCols, pipeLength, padding) {
        ...
        while(pixelsRows < this.height && counterRows <= numberRows) {
            ...
            let pipe = new Pipe(this.scene, pixelsCols, pixelsRows, 0, 0, 0, pipeLength, 0xffffff)
            this.add(pipe)
    ...
    createRows(numberRows, numberCols, pipeLength, padding) {
        ...
        while(pixelsCols < this.width && counterRows <= numberRows) {
            ...
            let pipe = new Pipe(this.scene, pixelsCols + pipeLength / 2 + padding / 2, pixelsRows - pipeLength / 2 - padding / 2, 0, 0, pipeLength, 0, 0xffffff)
            this.add(pipe)
