...
export default class CrepeGame extends Phaser.Scene {
    constructor(config) {
        ...
        this.crepeWidth = window.innerWidth/15;
        ...
        this.xOffset = window.innerWidth/6
        this.yOffset = 130;
    ...
    create() {
        ...
        let bg  = this.add.rectangle(0, this.halfW, window.innerWidth * 3, window.innerHeight*3, 0xFFFFFF);
    ...
    generateValidations() {
        ...
        this.validateBtn = this.add.image(this.screenWidthHalf, this.screenHeightSixth * 5, 'validate');
    ...
    generateActions() {
        this.arrowSelect = this.add.image(this.xOffset + this.screenWidthHeight - 50, this.yOffset, 'arrow');
    ...
    generateCrepes() {
        ...
        for (let i = 0; i < this.crepesAmount; i++) {
            let crepeWidth = this.crepeWidth + this.shuffledValues[i] * this.crepeWidth
            let posX = this.xOffset + this.screenWidthHeight + crepeWidth / 2
            let posY = i * this.crepePadding + this.yOffset + this.crepePadding / 2

            let crepe = new RoundRectangle(this, posX, posY, crepeWidth, this.crepeHeight, 15, /*this.hexValues[this.shuffledValues[i]]*/ /*0x580FA7*/ 0x58d874)
    ...
    flipEvent() {
        ...
        this.crepes.forEach((crepe, index) => {
            let posX = this.xOffset + this.screenWidthHeight + crepe.width/2
