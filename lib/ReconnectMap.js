...
class ReconnectMap extends Phaser.GameObjects.Container {
    ...
    createCols(numberRows, numberCols, pipeLength, padding, destroyedPipes) {
        ...
        while(counterRows + 1 <= numberRows) {
            ...
            if(!ReconnectMap.destroyedPipesCol().includes(counter) && counter != 1) {
                let pipe = new Pipe(this, this.scene, pixelsCols, pixelsRows, 0, pipeLength)
                this.add(pipe)
    ...
    createRows(numberRows, numberCols, pipeLength, padding, destroyedPipes) {
        ...
        while(counterRows <= numberRows) {
            ...
            if(!ReconnectMap.destroyedPipesRow().includes(counter) && counterRows != numberRows) {
                let pipe = new Pipe(this, this.scene, pixelsCols + pipeLength / 2 + padding / 2, pixelsRows - pipeLength / 2 - padding / 2, pipeLength, 0)
                this.add(pipe)
                this.pipes.push(pipe)
            }
    ...
    createLevels(levels) {
        levels.forEach(level => {
            ...
            let lvl = new LevelBox(this, this.scene, x, y, level)
    ...
    pipeClicked(pipe) {
        if(this.scene.game.availablePipes > 0 && (pipe.canBeUnlocked() || this.pipes.filter(p => !p.locked).length == 0)) {
            pipe.getClosePipes()
            pipe.unlock();
            this.scene.game.placePipe(this)
            
            this.levels.forEach(lvl => {
                if(lvl.canBeUnlocked() && lvl.locked) {
                    lvl.unlock()
                }
            })
        }
