...
class LevelBox extends Phaser.GameObjects.Container {
    ...
    unlock() {
        this.remove(this.iconLock)
        this.setText()
        this.box.fillColor = styles.difficultiesColors[this.difficulty]
        this.locked = false
    }

    // Check if a pipe unlocked is close to level
    canBeUnlocked() {
        let center = new Phaser.Math.Vector2(this.getBounds().x, this.getBounds().y)
        let dist = 7
        let filter = this.map.pipes.filter(p => 
            !p.locked && 
            center.distance(p.getTopLeft()) < dist ||
            center.distance(p.getTopRight()) < dist ||
            center.distance(p.getBottomLeft()) < dist ||
            center.distance(p.getBottomRight()) < dist
        )
